...
def page_editor(filtros):
    ...
    # ── Sincronizar ediciones del usuario (NO sobreescribir cambios programáticos) ──
    ...
    if isinstance(edited_df, pd.DataFrame) and not edited_df.empty:
        for col in all_cols:
            ...
            for i in range(min(len(dff), len(edited_df))):
                ...
                # Valor que se ENVIÓ al grid (snapshot)
                sent_val = dff.iloc[i][col]
                # Valor que el grid DEVUELVE
                grid_val = edited_df.iloc[i][col]
                sent_s = str(sent_val) if pd.notna(sent_val) else ""
                grid_s = str(grid_val) if pd.notna(grid_val) else ""
                # Solo sincronizar si el usuario cambió algo en el grid
                # (grid devuelve algo distinto a lo que se le envió)
                if grid_s != sent_s:
            ...
            # También recalcular si ENCOMENDACIONES en el df_main difiere del grid
            # (captura borrados o ediciones directas en la celda)
            if "ENCOMENDACIONES" in edited_df.columns and i < len(edited_df):
                cur_encom = str(st.session_state.df_main.at[real_idx_i, "ENCOMENDACIONES"]) if pd.notna(st.session_state.df_main.at[real_idx_i, "ENCOMENDACIONES"]) else ""
                cur_desc = st.session_state.df_main.at[real_idx_i, "Total Descuentos semanal (horas)"] if pd.notna(st.session_state.df_main.at[real_idx_i, "Total Descuentos semanal (horas)"]) else 0
                lookup = st.session_state.horas_lookup
                items = [x.strip() for x in cur_encom.split("|") if x.strip()]
                expected_td = sum(lookup.get(it, 0) for it in items)
                if abs(float(cur_desc) - expected_td) > 0.001:
                    ht = float(st.session_state.df_main.at[real_idx_i, "Horas Totales"]) if pd.notna(st.session_state.df_main.at[real_idx_i, "Horas Totales"]) else 0
                    st.session_state.df_main.at[real_idx_i, "Total Descuentos semanal (horas)"] = expected_td
                    st.session_state.df_main.at[real_idx_i, "Total Horas Clínicas"] = ht - expected_td
                    st.session_state.dirty_main = True

    # Auto-guardar en SQLite si hubo cambios y refrescar grid
    ...
    # ── Detectar fila seleccionada y llenar contenedores ARRIBA de la tabla ──
